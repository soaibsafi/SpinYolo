    ...
    try:
        ...
        if ACQ_MODE == 0:
            ...
            while(True):
                ...
                img = pImage.GetNDArray()
                t12 = time.time()
                #TODO Set input to the model
                detect_gray.image_detection(img, 0.5)
                ...
                print('ACQ Time: %s'%(td1*1000))
                print('Det Time: %s'%(td2*1000))
                print('TOT Time: %s'%(td3*1000))
                if td3 is not 0:
                    print('FPS: %s'%(1/td3))
                t13 = time.time()
